...
df_lat_lon = pd.read_csv(
    os.path.join(APP_PATH, os.path.join("data", "heartDisease_data_9001.csv"))
)
df_lat_lon["Heart Disease Value"] = df_lat_lon["Heart Disease Value"].apply(lambda x: str(x).zfill(5))

df_full_data = pd.read_csv(
    os.path.join(
        APP_PATH, os.path.join("data", "heartDisease_data_9001.csv")
    )
)
# df_full_data["county"] = df_full_data["county"].apply(
#     lambda x: str(x).zfill(5)
# )
df_full_data["county"] = (
    df_full_data["county"] + ", " + df_full_data.county.map(str)
)
    ...
    children=[
        html.Div(
            ...
            children=[
                ...
                html.H4(children="Heart Disease Values by County"),
                html.P(
                    id="description",
                            ...
                            options=[
                                {
                                    ...
                                    "value": "show_absolute_deaths",
                                },
                                {
                                    "label": "Histogram of total number of cases by county",
...
def display_map(figure):
    cm = dict(zip(DEFAULT_COLORSCALE))
...
@app.callback(Output("heatmap-title", "children"), [Input("value")])
...
def display_selected_data(selectedData, chart_dropdown):
    ...
    if len(fips) > 500:
        fig["layout"][
            "title"
        ] = "Age-adjusted death rate per county <br>(only 1st 500 shown)"
